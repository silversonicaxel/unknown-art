...
describe('Helpers > Utils > getTranslationServer', () => {
  ...
  test('should return expected resolved language', async () => {
    const { i18n: serverI18n } = await getTranslationServer(languageData)

    expect(serverI18n.resolvedLanguage).toBe('en')
  })

  test('should return expected change language function', async () => {
    const { i18n: serverI18n } = await getTranslationServer(languageData)
  ...
  test('should return expected t function', async () => {
